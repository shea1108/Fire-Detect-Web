...
def recover_password():
    ...
    html_body = f"""
    <table style="width:100%; max-width:600px; margin:0 auto; font-family:Arial,sans-serif; background:#f9f9f9; padding:20px; border-radius:8px;">
        <tr><td style="text-align:center;"><h2 style="color:#e63946;">🔐 Yêu cầu đặt lại mật khẩu</h2></td></tr>
        <tr><td>
            <p>Xin chào <strong>{user.user_name}</strong>,</p>
            ...
            <br /><div style="text-align:center; margin:20px 0;">
                <a href="{reset_link}" style="background:#1d3557; color:#fff; padding:12px 24px; border-radius:4px; text-decoration:none; font-weight:bold;">Đặt lại mật khẩu</a>
            </div><br />
...
def submit_reset_password(token: str):
    ...
    # <<< SỬA LỖI: So sánh thời gian cùng múi giờ để tránh lỗi >>>
    if (not user or
            user.user_reset_token != token or
            user.user_reset_expire_at is None or
            user.user_reset_expire_at < datetime.now(ZoneInfo('Asia/Ho_Chi_Minh'))):
        return jsonify(success=False, message='Token đã hết hạn hoặc không khớp'), 400
